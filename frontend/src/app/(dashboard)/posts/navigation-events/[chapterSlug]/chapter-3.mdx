...
navigation handler while remaining agnostic of how the url must be changed.
...
3. The navigation handler finds the navigation page (`page`) for the current url that implements `navToPost`.
...
- in `UrlRouter` we install the `DashboardNavPage` component (that will be mounted when the user
  visits the `/posts` page) and the `ArchiveNavPage` component (that will be mounted when the user
  visits the `/archive` page).
