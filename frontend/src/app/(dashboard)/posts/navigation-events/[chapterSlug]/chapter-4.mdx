...
clear what their relationship should be. In my applications, I consider the url to be the
single source of truth for *initializing* the application. When the application
...
Let's consider the case where the application is loaded. The url will point to a page (e.g. /blog-posts)
that requires a certain state. This state includes data and meta-data (e.g. the id of
the highlighted blog-post). Based on the current url:

- data needs to be fetched (I discuss this in [this post](/posts/state-providers/chapter-1));
- we must ensure that the correct post is highlighted.

Once the application is initialized then we must update the url when the highlighted post changes.
