...
In the "default properties" approach a component declares two types which - by convention - are called PropsT and DefaultProps.
The component receives a list of properties `props` that has been enriched by merging in the default properties:
...
const MyComponent = withDefaultProps<PropsT, typeof DefaultPropsT>(
...
- how can we use a value other than the default value in the child component?
...
component that creates the `defaultProps` object) will be re-rendered when `foo.bar` changes, even though `MyFrame` is not really
using `foo.bar`. By using a getter function we avoid referencing `foo.bar` during the render of `MyFrame`.

## What happens when a default property is overwritten?

We just saw how we can set `color="green"` on a `MyComponent` instance to override the default value of `color`. What happens if this
`MyComponent` has nested child components? Do these child components by default use `color=red` or `color=green`?
Most likely, you would want the latter option. For this reason, when `withDefaultProps` detects that you are overriding a default property,
...
extends and overrides the default properties of its parent `DefaultPropsProvider` instance. In the example below, we add an additional instance
of `MyComponent` with name "exampleInner". Because this instance is wrapped in another `<DefaultPropsProvider>` it will use "blue" as the default
color instead of "red", and it will have access to both `bar` and `baz`.
