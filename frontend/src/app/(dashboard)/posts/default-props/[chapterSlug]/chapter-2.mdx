...
value directly into `defaultProps` then MobX would notice that `MyFrame` is referencing it. That would mean that `MyFrame` will be
re-rendered when `foo.bar` changes, even though it's not really using `foo.bar`. By using a getter function we avoid all this.
