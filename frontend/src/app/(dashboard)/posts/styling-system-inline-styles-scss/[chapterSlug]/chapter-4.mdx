...
that all buttons have `select-none` and remove the need to add this class to every theme.
...
export const DialogButton = (props: PropsT) => {
  ...
  const name = props.themeClassName ? (props.themeClassName + '-DialogButton') : 'DialogButton';
